...
//    printf("Data:%f,%f,%f,%f\r\n",angleRange, rawAngleSpeed, firMDVal, intMode2Prediction);
    return NO_ERROR;
}
...
errorTypes getUpdAngleSpeed(double *angleSpeed)
{
    ...
    uint16_t firMDVal = 0;
    uint16_t intMode2Prediction = 0;
    readUpdAngleSpeed(&rawAngleSpeed);
    getAngleRange(&angleRange);
    readIntMode1(&firMDVal);
    readIntMode2(&intMode2Prediction);
    if (intMode2Prediction & 0x0004)
...
float ReadFromSensor(uint16_t command)
{
  ...
  if (command == READ_ANGLE_VAL_CMD){
    readreg = (readreg & (DELETE_BIT_15));
    return (float)readreg * 360.0f / (DELETE_BIT_15);
  }
  else if (command == READ_ANGLE_SPD_CMD){
    readreg = (readreg & (DELETE_BIT_15));
    uint16_t sign = (readreg & (SYSTEM_ERROR_MASK));
    if (sign == SYSTEM_ERROR_MASK){
